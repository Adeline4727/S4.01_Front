...
<template>
    <div class="deposer-une-annonce">
        ...
        <aside class="tips">
            ...
            <br>
            <p v-if="step == 1">Votre annonce sera trouvée plus facilement ! Vous aurez 50 % de chances en plus d’être contacté si votre annonce est bien nommées.</p>
            <p v-else-if="step == 2">Renseigner une adresse précise pour mettre en valeur votre bien. Votre futur vacancier se projettera bien plus si l’adresse est précise.</p>
            <p v-else-if="step == 3">Mettez en valeur votre annonce ! Plus il y a de photos, plus il sera facile de s'y projeter.</p>
            <p v-else-if="step == 4">Mettez en valeur votre annonce ! Plus il y a de détails, plus vos futurs contacts vous trouveront rapidement.</p>
            <p v-else-if="step == 5">Mettez en valeur votre bien ! Plus il y a de détails, plus votre annonce sera de qualité. Détaillez ici ce qui a de l'importance et ajoutera de la valeur.</p>
            <p v-else>Vous le savez, le prix est important. Soyez juste, mais ayez en tête une marge de négociation si besoin.</p>
        </aside>
    </div>
    <div v-if="showSuccessModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <h2>Félicitations !</h2>
            <p>Votre annonce <strong>{{ newAnnonce.titreAnnonce }}</strong> a été publiée avec succès. Elle est maintenant visible par les voyageurs.</p>
            <Button @click="fermerEtRediriger">Retour à l'accueil</Button>
        </div>
    </div>
</template>

<style scoped>
/* --- CSS DE LA POP-UP DE SUCCÈS --- */
    .modal-overlay {
        position: fixed;
        top: 0;
        left: 0;
    ...
    .modal-content {
        ...
        animation: apparitionModal 0.3s ease-out forwards;
    }
